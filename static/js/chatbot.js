...
class ChatWidget {
    ...
    renderCourses(courses) {
        ...
        courses.slice(0, 6).forEach(course => {
            ...
            card.style.cssText = 'padding: 30px; border-radius: 20px; transition: var(--transition); border: 1px solid var(--glass-border);';
            card.innerHTML = `
                ...
                <p style="color: var(--text-muted); margin-bottom: 15px; font-size: 0.9rem;">Master the skills of the future in our ${course.duration_months}-month intensive program.</p>
                <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--glass-border); pt: 15px; mt: 15px;">
                    <span style="font-weight: 700;">₹${parseFloat(course.base_fee).toLocaleString()}</span>
                    <span style="font-size: 0.8rem; color: var(--primary);">${course.status}</span>
                </div>
            `;
